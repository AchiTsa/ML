...
def print_overview(data_frame, file=''):
    if file:
        ...
        sys.stdout = open(  # noqa # pylint: disable=consider-using-with
            file, 'w', encoding='utf8')  # noqa # pylint: disable=consider-using-with
...
def print_categorical(data_frame, columns=None, file=''):
...
def print_nan_counts(data_frame):
    for column in data_frame.columns:
        if (nan_sum := data_frame[column].isna().sum()) > 0:
